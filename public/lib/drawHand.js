...
// Updated drawLandmarks function
v("drawLandmarks", function (a, c, b) {
    if (c) {
      ...
      for (var f = c.next(); !f.done; f = c.next()) {
        var landmark = f.value;
        if (landmark && (void 0 === landmark.visibility || landmark.visibility > b.visibilityMin)) {
          var x = landmark.x * d.width * factorX; // Apply factorX to x
          var y = landmark.y * d.height * factorY; // Apply factorY to y

          a.fillStyle = y(b.fillColor, { index: e, from: landmark });
          a.strokeStyle = y(b.color, { index: e, from: landmark });
          a.lineWidth = y(b.lineWidth, { index: e, from: landmark });

          var g = new Path2D;
          g.arc(x, y, y(b.radius, { index: e, from: landmark }), 0, 2 * Math.PI);
          a.fill(g);
          a.stroke(g);
          ++e;
        }
  ...
  // Updated drawConnectors function
  v("drawConnectors", function (a, c, b, d) {
    if (c && b) {
      ...
      for (var g = b.next(); !g.done; g = b.next()) {
        var connector = g.value;
        var start = c[connector[0]];
        var end = c[connector[1]];

        if (start && end && (void 0 === start.visibility || start.visibility > d.visibilityMin) && (void 0 === end.visibility || end.visibility > d.visibilityMin)) {
          var startX = start.x * e.width * factorX; // Apply factorX to startX
          var startY = start.y * e.height * factorY; // Apply factorY to startY
          var endX = end.x * e.width * factorX; // Apply factorX to endX
          var endY = end.y * e.height * factorY; // Apply factorY to endY

          a.strokeStyle = y(d.color, { index: f, from: start, to: end });
          a.lineWidth = y(d.lineWidth, { index: f, from: start, to: end });
